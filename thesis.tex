...
The result in \textbf{Eq. \ref{functional3}} is useful to derive the solution in the following sections.
...
\begin{equation}\begin{split}
&E^\textit{H} = \sumi<i> <\bwff<i><i> |\ -\frac{\nabla^{2}_{i}}{2} + V_\textit{ext}(\textbf{r}_{i})  \ | \bwff<i><i> > \\
& +\frac{1}{2} \suminj<i><j> <\bwff<i><i> \bwfnn<j><j>|\ \frac{1}{|\textbf{r}_{i} - \textbf{r}_{j} |} \ | \bwff<i><i> \bwfnn<j><j>>.
\end{split}\end{equation}
...
\begin{equation}\label{haa}
 \frac{\delta(E^{\textit{H}} - \sumi<i> E_{i}^{H} (<\bwff<i><i> | \bwff<i><i> > -1))}{\delta \phi_{i}^*({\textbf r})}  = 0. 
\end{equation}

where $(<\bwff<i><i> | \bwff<i><i> > -1)$ means that the wavefunction is normalized, and the minimization is with respect to ${\delta \phi_{i}^*({\textbf r})}$.

\textbf{Eq. \ref{haa}} is calculated by derivative of functionals, which is given as

\begin{equation}\label{variation1}
 \frac{\delta E^{\textit{H}} }{\delta \phi_{i}^*({\textbf r})}  = E_{i}^{H} \bwff<i><i>
\end{equation}


The left term in \textbf{Eq. \ref{variation1}} can be calculated by two parts using \textbf{Eq. \ref{functional3}}. The first part is 

\begin{equation}\begin{split}\label{hartree1}
& \frac{\delta ( \sumi<i> <\bwff<i><i> |\ -\frac{\nabla^{2}_{ i}}{2} + V_\textit{ext}(\textbf{r}_\textbf{i}))  \ | \bwff<i><i> > )}{\delta \phi_{i}^*({\textbf r})}  \\
&  =  (-\frac{\nabla^{2}_{ i}}{2} + V_\textit{ext}(\textbf{r}_\textbf{})) \bwff<i><>
\end{split}\end{equation}

The second part is

\begin{equation}\begin{split}\label{hartree2}
&  \frac{1}{2} \suminj<j><i> \frac{\delta( <\bwff<i><i> \bwff<j><j>|\ \frac{1}{|\textbf{r}_{i} - \textbf{r}_{j} |} \ | \bwff<i><i> \bwff<j><j>>)}{\delta \phi_{i}^*({\textbf r})}   \\
& =  \suminj<j><i> <\bwff<j><j>|\ \frac{1}{|\textbf{r} - \textbf{r}_{j} |} \ |  \bwff<j><j>>  \phi_{i} (\textbf{r}) \\
& = \suminj<j><i> <\phi_{j} (\textbf{r'})|\ \frac{1}{|\textbf{r} - \textbf{r'} |} \ |  \phi_{j} (\textbf{r'}) >)  \phi_{i} (\textbf{r})
\end{split}\end{equation}

Therefore, \textbf{Eq. \ref{variation1}} is 
