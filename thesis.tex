...
& {T} = - \sum_i^{Ne}   \frac{{{\nabla}_{\textit{i}}^{2}}}{2} \\
& U_{ext} =  \sum_i^{Ne} \sum_J^{Nn} \frac{Z_\textit{I}}{|\textbf{r}_\textit{i}-\textbf{R}_\textit{I}|} \\
& {U}_\textit{int} =  \frac{1}{2} \sum_i^{Ne} \sum_{j \neq i}^{Ne} \frac{1}{ |\textbf{r}_\textit{i}-\textbf{r}_\textit{j}|} \\
& {{H}_{zz}} = \frac{1}{2} \sum_I^{Nn} \sum_{J \neq I}^{Nn} \frac{Z_\textit{I} Z_\textit{J}\ }{|\textbf{R}_\textit{I}-\textbf{R}_\textit{J}|}.
\end{split}
\end{equation}
...
\begin{equation}\begin{split}
 &  {H}_{zz1} = - \sum_I^{Nn} \frac{{{\nabla}_{\textit{I}}^{2}}}{2 M_\textit{I}}   \\
 &  {H}_{zz2} = - \sum_I^{Nn} \frac{1}{M_\textit{I}} {\int {\Psi'(\{\textbf{r}_{\textit{i}}\}; \{\textbf{R}_{\textit I}\})}^{\ast} {{\nabla}_{\textit{I}}} {{\Psi'(\{\textbf{r}_{\textit{i}} \}; \{\textbf{R}_{\textit I}\})}} d {\textbf r }} {{\nabla}_{\textit{I}}}  \\
 &  {H}_{zz3} = - \sum_I^{Nn} \frac{1}{M_\textit{I}} {\int {{\Psi'(\{\textbf{r}_{\textit{i}}\}; \{\textbf{R}_{\textit I}\})}}^{\ast} {{\nabla}_{\textit{I}}^{2}} {{\Psi'(\{\textbf{r}_{\textit{i}} \}; \{\textbf{R}_{\textit I}\})}} d {\textbf r }}. 
\end{split}\end{equation}
...
Here, $i$ goes through all the electrons. $\bwff<i><i>$ implies state of the $i$:th electron in the position of ${\textbf{r}}_{\textit{i}}$, where the different states in the same position are orthonormalized.
...
\begin{equation}\begin{split}
&  {\hath} = \sum_i^{Ne} \Big( \frac{{{\nabla}_{\textit{i}}^{2}}}{2} + \sum_I^{Nn} \frac{Z_\textit{I}}{|\textbf{r}_\textit{i}-\textbf{R}_\textit{I}|} \Big) +   \frac{1}{2} \sum_i^{Ne} \sum_{j \neq i}^{Ne} \frac{1}{ |\textbf{r}_\textit{i}-\textbf{r}_\textit{j}|} \\
& = \sum_i^{Ne} h_1(\textbf{r}_\textit{i}) + \frac{1}{2} \sum_i^{Ne} \sum_{j \neq i}^{Ne} h_2(\textbf{r}_\textit{i},\textbf{r}_\textit{j}).
\end{split}
\end{equation}
...
\begin{equation}\begin{split}
& E^\textit{H} = \sum_i^{Ne} <\bwff<i><i> | \  h_1(\textbf{r}_\textit{i}) \ | \bwff<i><i> > \\
& +\frac{1}{2} \sum_i^{Ne} \sum_{j \neq i}^{Ne} <\bwff<i><i> \bwff<j><j>|\ h_2(\textbf{r}_\textit{i},\textbf{r}_\textit{j}) \ | \bwff<i><i> \bwff<j><j>>.  
\end{split}\end{equation}
...
Furthermore, the constraint is that the different states in the same position are orthonormalized. According to method of Lagrange multipliers, the variation with  Lagrange multiplier $E_{i,j}^{H}$ is satisfied

\begin{equation}\label{haa}
 \delta \Big(E^{\textit{H}} - \sum_i^{Ne} \sum_j^{Ne} E_{i,j}^{H} (<\bwff<i><i> | \bwff<j><j>> -\ \delta_{ij})\Big)  = 0. 
\end{equation}
...
&  \delta ( \sum_i^{Ne} <\bwff<i><i> |\ h_1(\textbf{r}_\textit{i})  \ | \bwff<i><i> > )  \\
...
& \frac{1}{2} \delta \sum_i^{Ne} \sum_{j \neq i}^{Ne} <\bwff<i><i> \bwff<j><j>|\ h_2(\textbf{r}_\textit{i},\textbf{r}_\textit{j}) \ | \bwff<i><i> \bwff<j><j>> \\
& = \frac{1}{2}  \sum_{i \neq k}^{Ne} \Big( <\bwff<i><i>  \delta \bwff<k><>|\ h_2(\textbf{r}_\textit{i},\textbf{r}_\textit{}) \ | \bwff<i><i> \bwff<k><>> \\
& + <\bwff<i><i>  \bwff<k><>|\ h_2(\textbf{r}_\textit{i},\textbf{r}_\textit{}) \ | \bwff<i><i> \delta \bwff<k><>> \Big) \\
& + \frac{1}{2}  \sum_{j \neq k}^{Ne} \Big( <\delta \bwff<k><>  \delta \bwff<j><j>|\ h_2(\textbf{r}_\textit{},\textbf{r}_\textit{j}) \ | \bwff<k><> \bwff<j><j>> \\
...
& \delta \sum_i^{Ne} \sum_j^{Ne} E_{i,j}^{H} \Big(<\phi_{i}(\textbf{r}_\textit{i}) | \phi_{j}(\textbf{r}_\textit{j}) > - \ \delta_{i,j}\Big) \\
& = \sum_j^{Ne} E_{k,j}^{H} \Big(< \delta \phi_{k}(\textbf{r}) | \phi_{j}(\textbf{r}_\textit{j}) > \Big) + \sum_i^{Ne} E_{i,k}^{H} \Big(<\phi_{i}(\textbf{r}_\textit{i}) | \delta \phi_{k}(\textbf{r}_\textit{}) >\Big) \\
& = \sum_i^{Ne} E_{k,i}^{H} \Big(< \delta \phi_{k}(\textbf{r}) | \phi_{i}(\textbf{r}_\textit{i}) >  + <\phi_{i}(\textbf{r}_\textit{i}) | \delta \phi_{k}(\textbf{r}_\textit{}) >\Big)
\end{split}
\end{equation}
...
\begin{equation}\begin{split}\label{hha1}
 \frac{ \delta  \Big(E^{\textit{H}} - \sum_i^{Ne} \sum_j^{Ne} E_{i,j}^{H} (<\bwff<i><i> | \bwff<j><j>> -\ \delta_{ij})\Big)}{\delta \phi^*_{k}(\textbf{r})}  = 0. 
\end{split}
\end{equation}

Therefore, the \textbf{Eq. \ref{hha1}} can be derived as 

\begin{equation}\label{hartreq}
\Big(-\frac{\nabla^{2}_{k}}{2} + \sum_I^{Nn} \frac{Z_\textit{I}}{|\textbf{r}_\textit{}-\textbf{R}_\textit{I}|} + \sum_{j \neq k}^{Ne}  <\bwfn<j><'>\ | \frac{1}{|\textbf{r} - \textbf{r}^{'} |} \ | \bwfn<j><'>>\Big) \bwff<k><> = \sum_i^{Ne} E_{k,i}^{H} \bwff<i><> 
\end{equation}

There are many solutions to the \textbf{Eq. \ref{hartreq}}, each corresponding to a different set of $E_{k,i}^{H}$. One can choose to $E_{k,i}^{H}$ which satisfies $E_{k,i}^{H}$ = $\delta_{k,i}\epsilon_k$. The \textbf{Eq. \ref{hartreq}}
can be rewriteen as

\begin{equation}\begin{split}\label{hartreq}
& \Big(-\frac{\nabla^{2}_{k}}{2} + \sum_I^{Nn} \frac{Z_\textit{I}}{|\textbf{r}_\textit{}-\textbf{R}_\textit{I}|} + \sum_{j \neq k}^{Ne}  <\bwfn<j><'>\ | \frac{1}{|\textbf{r} - \textbf{r}^{'} |} \ | \bwfn<j><'>>\Big) \bwff<k><> = \epsilon_k \bwff<k><>  \\
&  \qquad \qquad \qquad  \qquad \qquad \qquad \qquad \qquad  \Downarrow  \\
& \Big(-\frac{\nabla^{2}_{i}}{2} + \sum_I^{Nn} \frac{Z_\textit{I}}{|\textbf{r}_\textit{}-\textbf{R}_\textit{I}|} + \sum_{j \neq i}^{Ne}  <\bwfn<j><'>\ | \frac{1}{|\textbf{r} - \textbf{r}^{'} |} \ | \bwfn<j><'>>\Big) \bwff<i><> = \epsilon_i \bwff<i><>  \\
\end{split}
\end{equation}


Here, \textbf{Eq. \ref{hha1}} is a group of dependent single particle equations. $\epsilon_k$ is identified as the eigenvalue for this single-electron Hartree equation.
...
electron density $\rho$. Obviously, this will lead to two different Hamiltonians, that is, ${H}_{1}$ and ${H}_{2}$, as well as two different corresponding
wavefunctions named $\Psi_1$ and $\Psi_2$. Since $\Psi_1$ are not the ground state wavefunction of ${H}_{2}$, the same rules to $\Psi_2$ and ${H}_{1}$, two following
inequality equations are satisfied as

\begin{equation}\label{hkpf1}\begin{split}
&  E_1 = \langle \Psi_1\ |{H}_{1}|\ \Psi_1 \rangle  \ < \  \langle \Psi_2\ |{H}_{1}|\ \Psi_2 \rangle,\\
&  E_2 = \langle \Psi_2\ |{H}_{2}|\ \Psi_2 \rangle  \ < \  \langle \Psi_1\ |{H}_{2}|\ \Psi_1 \rangle.
\end{split}\end{equation}

Taking advangtage of the Hamiltonian from \textbf{Eq. \ref{both}}, one can get
\begin{equation}\label{hkpf2}\begin{split}
&    \langle \Psi_2\ |{H}_{1}|\ \Psi_2 \rangle \\
&  = \langle \Psi_2\ |{H}_{2} + {H}_{1} - {H}_{2}|\ \Psi_2 \rangle \\
&  = \langle \Psi_2\ |{H}_{2} |\Psi_2 \rangle + \langle \Psi_2 | {H}_{1} - {H}_{2}|\ \Psi_2 \rangle \\
...
Another similar inequality equation can be gained if one changes the equation $\langle \Psi_1\ |{H}_{2}|\ \Psi_1 \rangle$ like \textbf{Eq. \ref{hkpf2}}
