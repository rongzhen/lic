...
telluride (CdTe) and chalcopyrite copper indium gallium selenide (CIGS). The maximum conversion efficiency of CIGS had reached 23.3\% in the National Renewable Energy Laboratory, Golden, Colorado, U.S.A. Therefore,
...
{USE SAME STYLE FOR v1, v2, c3 as in main text and in papers}


\noindent 
In this licentiate thesis, details in the electronic structure of CuIn\textsubscript{1-x}Ga\textsubscript{x}Se\textsubscript{2} alloy ($x$ = 0.0, 0.5 and 1.0) and its optical response are explored by means of the all-electron and full-potential linearized augmented plane wave method in conjunction with the density functional theory. 
The energy band dispersion are parameterized for the three uppermost valence bands (VBs; v1 to v2) and the lowest conduction band (CB; c1), based on the $\mathbf k \cdot \mathbf p$ method but expanded up to high order. 
...
but also an energy dependent effective mass which together better can describe both the transport and band filling effect
...
We find that v1$-$c1 and v2$-$c1 transitions in the Brillouin zone edge are responsible for the main absorption peaks
at 3.00 and 3.09 eV, where v1 and v2 represent the highest valence band and the second highest valence band, respectively. However, also the energetically lower VBs contributes to the high absorption coefficient. 
...
flexible and result in lower weight. The maximum conversion efficiency for thin-film solar cells is lower than that for crystalline silicon today, which is 23.3\% using copper indium gallium selenide (CIGS) with concentrator by NREL \cite{ward2014cu} and 
...
should be abundant and non-toxic as well. In this section, four main solar cell materials are discussed briefly: silicon (Si), gallium arsenide (GaAs), cadmium telluride (CdTe), copper indium gallium selenide (CIGS).
...
materials are discussed in this section: amorphous silicon (a-Si), cadmium telluride (CdTe), and copper indium gallium selenide (CIGS).
...
\section{Copper indium gallium selenide (CIGS)}
...
Therefore, plane waves are suitable to describe wavefunction in $I$ region (\textbf{Eq. \ref{ap111}}). The drawback of APW method is that the wavefunction is dependent with energy, which leads to the 
...
\noindent 
{Check abbdreviation in the text. Also, if abbreviation is well-known, use abbreviation without re-defining them.
Why not use the abbreviation CIGS and CZTS ??}

{GO THROUGH THE TEXT AND make consistent how you/we use CIGS and CZTS}
{please correct my English, I have done this quite quickly}

The most fundamental propoety of a material  is the energy of the electron. That is determined by solving the Kohn-Sham equation (see Eq ??? in Chapter ??). By understanding the electronic properties of the electrons one can understand 
many of the fundamental material properties. During the last two decades the electronic structure of CIGS has been explored, however the motivation of the present study is that much of the earlier studies describe the electronic structure and the corresponding density-of-states (DOS) on a wider energy scale, typically between 5-10 eV below valence bands maximum (VBM) to 5-10 eV above conduciton band minimum (CBM). With that perspective one gets the overall understanding of the material, and one also gets the bond character, but details near the band edges are less revealed. The reason why details are importand can be understood from different energy of relevance in materials. The energy to create defects is typically on the eV-scale (a Cu vacancy costs about 0.8 eV to be form in CIS), the energy of band filling in heavly doped, degenerated semiconductors are typically in the 0.1 eV scale, and room temperature corresponds to $k_BT $ ~ 0.025 eV. Thus, to understand band filling and transport of electrons/holes one has to understand the electronic structure on an energy scale of 1 meV to ~0.1 eV.    
Moreover, most of the ealier studies are on the ternary compounds CuInSe\textsubscript{2} and CuGaSe\textsubscript{2}, wheareas the experimentally and commercially most interesting compound is the CIGS alloy with about 70\% In and 30\%  Ga, that is Cu2In0.7Ga0.3Se2. It is therfore interesting to understand how Ga-on-In alloying affects the electronic structure.   

In this paper, we have therefore conducted a study on CuIn\textsubscript{1-\textit{x}}Ga\textsubscript{\textit{x}}Se\textsubscript{2}, with $x$ = 0, 0.5, and 1. Here, $x$ = 0.5 is not exactly the same configuration as for the comercially interesting compound, but it indicated how CIGS behaves when alloying In and Ga. Moreover, the present study focuses on the details in the electronic structure near the band edges. We do that by analyzing
 the lowest conduction band (CB) and the three uppermost valence bands (VBs) of CIGS ($x$ = 0, 0.5, and 1)
for the FPLAPW calculations (Chapter ??) of the electronic band structure. The advantagous of using FPLAPW within the Wien2k package is that the codes generate very accurate energy values in terms of noise and level of degeneracy of states, while the disadvantage is that the code does not have same possibility of choosing post-DFT calculations. Having accurate description of the degenerate states at $\Gamma$-point VBM is crucial when determinig the effective hole masses there since one needs an accuracy of 0.01--0.1 meV. 
Therefore, we follow the FPLAPW calculation approach described in Ref. ?? {my JAP} which has shown to be able to overcome the problem with unwanted interaction between VB and CB states which can strongly affect the value of effective masses (Ref. my APL paper). It has also been proven to generate accurate values of the effective masses (Ref. my Si/SiC), and our results for effective electron mass of CIS ($m_c$ ~ ?? $m_0$) is close the the experimetal results (??) by ??? {take data/info from our paper}.   
Thus, even if we do not generate the band structure with the exact exchange-correlation potential, we have reason to believe that the results are fairly good, and that the results is useful when enalyzing future measurements and simulations.

In this paper we present parameterization of electronic band structure of the lowest CB and the three uppermost VBs. These bands are the most important bands for electorn and hole transort in the material. With the parameterization of the bands, other researchers can easily generate the energy bands in order to further investigate properties that are related to band filling and carrier transport. 

The parameterization of the energy bands
demonstrates that the energy dispersions of the lowest CB and three uppermost VBs are strongly anisotropic and non-parabolic close to the $\Gamma$-point. This anisotropy and non-parabolicity directly affect the effective
electron and hole masses. Thereby we can better understand the CIGS . 
This parameterization method can be utilized to other materials potentially, such as kesterite and stannite 
...
Here, the effective mass tensor (see Eq. ? in Chapter ?) is directly related to the curvature of the bands. The effecive mass of the electron (or hole) is a very fundamental property of a semiconductors and is used for various analysis of experiments {an opponent can ask which} and carrier transport simulations. Low effective mass value normally means better transport of the carriers, this is very important for the minority carriers (i. e., the electrons in $p$-type CIGS).  
In addition, and in models the effective mass is often used to approximately describe the band structure around the badn edge, for instance to model band filling. Thus, the effective mass is used in two ways: to describe transport of a single carrier and to describe collective band filling. 
The effective masses are normally presented (both experimentally and theoretically) as constant value(s) at the very minimum of the CBM or the very maximum of the VBM. That is correct if one can measure (or accurately calculate) details in the electronic structure near the band edges. However, very often one measure effective masses indirectly and for a material with high electron and hole concentration. Therefore the measured mass value is not primarily the mass at the very CBM or VBM but more the masse near the quasi Fermi level. We therefore describe the effective mass both as the $\Gamma$-point effective mass of the CB and the three VBs, but also an average mass that are energy dependent and shall better represent the effective mass in models that describe transport. That is, one can use the energy-dependent mass $m(E)$ in existing models for the approximative models and a little better describe the physics of the material.      

In Fig. XX (which is Fig 3 in the paper), we illustrate how strongly the effective mass of the bands depends on specific $k$-state that one considers. The $\Gamma$-point effective mass (value when wave vector equals 0) is only valid close to the$\Gamma$-point. Already some 5-10/% away from the $\Gamma$-point the mass value has changed. This is very obvious for the second uppermost VB (red lines) in CIS. One can notice that the behaviour of the effective electron masses are similar 
while it differs much more for the effective holes masses between of the three compounds (i.e., $x$ = 0, 0.5, and 1.0). 
This is a direct consequense of the different spin-orbit split-off and the crystal-field split off energies. Both spin-orbit interaction and the crystal field split states at the $/Gamma$-point that otherwise would have been degenerate. These splits directly affect the band curvatures and therefore the effective masses. This is discussed in the paper.    
Moreover, in the figure one can see that the effective mass is very anisotropic, that is, different in different $k$ directions; this is most obvious for the holes in the VBs. That means that carrier transport will be differnet in different$k$ directions.  
...
better analyze the impact on density-of-states (DOS) as well as carrier concentrations and Fermi energy due
by taking into account the non-parabolicity and anisotropy of the energy bands. 

 
That is, in this work we explore how temperature and doping affect the carrier concentraion and we compare the 
...
In the paper, we explore the DOS of the VB and the CB, and we find that the parabolic approximation very poorly describes the DOS for especially the VB. Normally one uses the effectice DOS mass to represent the parabolic description of the DOS, but that is thus not an accurate method for the VBs in CIGS. Therefore, we introduce and energy-dependent DOS mass that better can represent the energy-dependent DOS, and that can be used in the traditional models. That is, with the DOS mass one can describe the band filling better, still using models that have been derived for a constant value of the DOS mass. The advantage is that the energy-dependent mass easily can be use in analyses of measurements where band filling is strong.

Moreover, with the parameterized bands we analyse temperature dependence of the carrier concentrations ($n$ = $p$) in intrisic mateirla. As expected, the carrier concetration depends much on the band-gap energy of the material; the carrier concentration for Si (multi-valley CBM and $E_g$ $\approx$ 1.2 eV) is similar as in CIS ($E_g$ $\approx$ 1.0 eV) and the carrier concetration for GaAs ($E_g$ $\approx$ 1.5) lies between CIGS with $x$ = 0.5 ($E_g$ $\approx$ 1.3 eV) and CGS ($E_g$ $\approx$ 1.6 eV).       

We also model the temperature dependent Fermi levels and carrier concetrations in $p$-type CIGS 
since the material is use as $p$-type absorber layer in solar cells. Thus, an accurate description of the the temperature dependences are important in analyses and simulations of the material and devices. Here, we find that there is a large difference for the full band description compared with the parabolic approximation. The difference is explained by the 
the non-parabolicity and anisotropy of the energy bands.  
...
The most importand optical property of a solar cell material is how efficient the material is to absorb the sun-light. The absorption coefficient $\alpha(\omega)$ describes this efficiency in units of 1/length. That is, if the absorption coefficient is large the thickness of the material can be thinner and still the material absorb same amount of light. 
The absorption coefficient is related to the dielectric function through the expression
{if you don't have expression in thesis, then add here}. 
Here it is the imaginary part $\epsilon_2(\omega)$ of the dielectric function that is closely related to the absorption coefficient; for instance, both $\epsilon_2(\omega)$ and $\alpha(\omega)$ are zero for photon energies smaller than the enery gap.  That is, by analyzing the dielectrict function one can also understand how the optical transition occurs and
how good/bad the material is for a solar cell. That is, the imaginary part of the dielectric function is obtained from 

{insert the equation, yes again}
...
delta-Diract function in the equation) and the strenght of the absorption possibility which is described by the 
optical matrix element { give simple explression for that, like <$pki_kc$ |  grad   psi}. While the energy difference between final and initial state only depends on the electronic band structure, the optical matrix element depends on the shape and symmetry of the electron wave function. Certain band-to-band transition can be weak or strong depending on the wavefunction of the final and initial states. To understand the optical properties of a material, one needs to analyze the dielectric function in details.  
...
\noindent In this paper, we have therefore calculated the spectrum of the dielectric function for CuIn\textsubscript{0.5}Ga\textsubscript{0.5}Se\textsubscript{2} by means of the all-electron FPLAW method as in Papers I and II. 
The calculated dielectric function is compared with experimental data (CuIn\textsubscript{0.7}Ga\textsubscript{0.3}Se\textsubscript{2}) at temperatures of 40 K and 300 K, and we find that theory and experiments are in reasonable good agreement. We decompose the full dielectric funcition into the contribution from each band-to-band transition and for each state in the Brillouin zone.  The different contributions to imaginary part of dielectric function in terms of the transitions between the valence bands and the conduction bands are identified based on this
...
by experimentalist. The supprising conclusion is that there are different band-to-band trnasitions that are important and that several low-lying states in the VBs that strongly contribute to the total dielectric function. That is, a full band description is needed to accurately describe the optical activity of these materials. \end{enumerate}
...
Although the mateiral properties are know from earlier theoretical and experimental 
studies, detail in the electronic structure of the band edges are less studies, especially not for the alloy CuIn{\textsubscript{1-$x$}}Ga{\textsubscript{$x$}}Se\textsubscript{2} .
...
the electron and hole dynamics in the materials. We consider intrisic and $p$-type  materials, and we model
...
Here, the dielectric function spectra for the is calculated and is compared with experimental result. 
...
Overall, we find that it is important to consider a the full band description of the material to understand the
fundamental properties of the electronic structure and the optical activity. Especially, the energetically high
lying Cu $d$-states in CIGS plays an significan role since they hybridized with the In/Ga and Se states near the VBM and some 2-6 {CHECKKK} eV below the VBM.  Cu $d$-states form flat energy bands in the VB that contribute to a high optical efficiency of the compound. This hybridization make the VBs very non-parabolic, also for the uppermost VBs at least away from the $\Gamma$-point of the Brillouin zone. Therefor a parameterization of the bands is rather complex.  
Moreover, we find that it is crucial to include the spin-orbit interaction when analyzing details in the electronic struture. Both the spin-orbit split-off and the crystal field split-off make the uppermost VBs strongly non-parabolic also close to the $\Gamma$-point. The impact of the spin-orbit interaction and the crystal field is less important when studying properties related to total energy, like ion relaxation, defect formation energies, etc, but in studies related to excitation, band filling, and carrier transport one needs to consider these effects.        


\noindent 
For future perspective and from a theoretical point of view, improved analyses can be done when better computational methods have beed developed and are available. The LDA and GGA are fairly accurate method, but have limitations in terms of inaccurate band-gap energies and localization of d- and f-states. The GW approach that we employ today is very time-comsuming and it does not generate total energies; that limit the usability of the approach. 
Hybrid functionals (like HSE, REF???) gives better band gap energies, but this approach is as time-consuming as the GW approach which is a limitation for extensive research of complex materials. Moreover, there are discussions how accurate the approach is to models details in for instance defect transition energies. 
That is, further development of computatinal methods will help improving the results, however it is not certain that
...
and non-tixic elements in addition to have crystal stability and high optical absorption efficiency.  
...
and the material have more complex native defects. 

One can also speculate if there are other similar material that are of interest. CIGS and CZTS can be 
considered to be in a class denoted as Cu-XY-chalcogen, where X and Y are two cation elements that replaces the 
group-III In/Ga in CIGS. From the present study, we understand the advantage of having Cu as an element in the 
compound, but with choosing proper element for additional cation element ona can maybe find optimal material 
...
properties of these materials one can further understand the Cu-XY-chalcogen type compounds.    
Cu2O is the first absorber material for solar cells (REF), and this material is still considered as an interesting solar cell material if one can stabilize the structure. Cu2S and perhaps Cu2(O,S) is an alterlative to that type of material.

For all these material, and perhaps other Cu-XY-chalcogen compounds one needs to explore the electronic and optical properties in combination with studies of crystalline stability/degradation, defect formation, and dopability. 
By systmatically analyzing and understanding the materials one has better possibilities to tailor-make compounds with optimal performance for thin-film, and perhaps ultra-thin-film, solar cells.

  
 

{I think you shall be careful with not describing CuFS2 before you have a manuscript. 
You do not have to say what you will do in the future, only say what possibilities there are }
