...
\caption{Data is from BP statistical review of world energy 2014, METO means million tonnes oil equivalent.}
\label{wpec}
\end{figure}
...
\caption{Solar irradiance spectrum (raw data in stockholm, and I will modify the figure the unit in Y-axis and add something more, such as UV, infrared...).}
\label{spectrumsun}
\end{figure}
...
In the following \textbf{Fig. \ref{illu1}}, the SCR becomes more "smooth"
...
due to the extra load, and the stablized fermi level at the stable equilibrium split under illumination.  The chemical potential $\bigtriangledown \mu = E_{nf} - E_{vf}$ is created, 
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.5]{tree1.jpg} 
...
mobility, higher saturated electron velocity, absorb sunlight more efficiently due to the direct band gap. The optimum band gap for the single junction solar cell is suggested around 1.4 eV by theoretical calculations,
...
thin film materials are considered in the solar cells: amorphous silicon (a-Si), cadmium telluride (CdTe), copper indium gallium selenide (CIGS),  and copper zinc tin sulfide or selenide 
(CZT(S,Se)).
...
It is maybe also explained that the best solar cell is with the Ga content of 30\% (x=0.3), however, the band gap energy of the CIGS suggests that the optimum solar cell conversion efficiency is obtained with between x=0.5 
and x=0.7.
...
and the n-type ZnO is dopped by the aluminum (Al). This CIGS/CdS/ZnO strucutrure was found to improve the cell performance.
