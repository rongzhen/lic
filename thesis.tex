...
Here, $\varDelta_{SO}$ is the spin-orbit splitting energy of the valence band. The average effective mass $m_{SO}$ can be calculated from  \textbf{Eq. \ref{9054}} 
...
and Hamiltonian $\hbar\textbf{kp}/m_e$ is considered as perturbation. The following perturbation matrix is derived without consideration spin-orbit interaction
...
Here, $L$, $M$, and $N$ are electron momentum matrix elemente, and the corresponding 3$\times$3 matrix is written as $Y$ in \textbf{Eq. \ref{kpmatrix1}}. The energy dispersion can be obtained from $\lambda$ by
