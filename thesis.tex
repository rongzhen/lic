...
\begin{split}
& \hath = - \sumi<i> {\frac{\hbar^{2}}{2 m_{\textit e}}}   \nablaia<i> - \sumi<I> {\frac{\hbar^{2}}{2 m_\textit{I}}} \nablaia<I>  - \sum_i \sum_I \frac{Z_\textit{I}\ e^2}{4 \pi \varepsilon_0 |\textbf{r}_\textit{i}-\textbf{R}_\textit{I}|} \\
& + \frac{1}{2} \sum_i \sum_{j \neq i} \frac{ e^2}{4 \pi \varepsilon_0 |\textbf{r}_\textit{i}-\textbf{r}_\textit{j}|} + \frac{1}{2} \sum_I \sum_{J \neq I} \frac{Z_\textit{I} Z_\textit{J}\  e^2}{4 \pi \varepsilon_0 |\textbf{R}_\textit{I}-\textbf{R}_\textit{J}|}.
\end{split}
\end{equation}
...
constant ($\alpha$ = ${e^2}/{(4 \pi \varepsilon_0 c \hbar)}$). Therefore, the velocity of light in atomic units is $c$ = $1/{\alpha}$ and ${e^2}/{(4 \pi \varepsilon_0)}$ = 1. In atomic units, mass of the \textit{i}:th nucleus is $M_I$.
Therefore, \textbf{Eq. \ref{th}} is given as

\begin{equation}\label{sth}\begin{split}
&\hath = - \sumi<i>   \frac{{{\nabla}_{\textit{i}}^{2}}}{2} - \sumi<I> \frac{{{\nabla}_{\textit{I}}^{2}}}{2 M_\textit{I}}  - \sum_i \sum_I \frac{Z_\textit{I}}{|\textbf{r}_\textit{i}-\textbf{R}_\textit{I}|} \\
& + \frac{1}{2} \sum_i \sum_{j \neq i} \frac{1}{ |\textbf{r}_\textit{i}-\textbf{r}_\textit{j}|} + \frac{1}{2} \sum_I \sum_{J \neq I} \frac{Z_\textit{I} Z_\textit{J}\ }{|\textbf{R}_\textit{I}-\textbf{R}_\textit{J}|}.
...
& {\hath} = - \sumi<I> \frac{{{\nabla}_{\textit{I}}^{2}}}{2 M_\textit{I}} + {\hat{H}_{nn}} + {\hath}^{BO} \\
...
& \hat{T} = - \sumi<i>   \frac{{{\nabla}_{\textit{i}}^{2}}}{2} \\
& V_{ext} =  \sum_i \sum_J \frac{Z_\textit{I}}{|\textbf{r}_\textit{i}-\textbf{R}_\textit{I}|} \\
& \hat{V}_\textit{int} =  \frac{1}{2} \sum_i \sum_{j \neq i} \frac{1}{ |\textbf{r}_\textit{i}-\textbf{r}_\textit{j}|} \\
& {\hat{H}_{nn}} = \frac{1}{2} \sum_I \sum_{J \neq I} \frac{Z_\textit{I} Z_\textit{J}\ }{|\textbf{R}_\textit{I}-\textbf{R}_\textit{J}|}.
\end{split}
\end{equation}

Here, ${\hath}^\textit{BO}$  is the Hamiltonian for the electrons within the Born-Oppenheimer approximation. The subscript $ext$ implies $external$ in \textbf{Eq. \ref{nh}}, and $V_{ext}$ describes
...
\noindent Here, $E(\{{\textbf R_I}\})$ is the system total energy, which is ${\textbf R_I}$-dependent because system wavefunction depends on nuclei positions. One ends up with the following equation using \textbf{Eq. \ref{vext}}
...
& \Big( \hat{H}_{nn1}+\hat{H}_{nn2}+\hat{H}_{nn3}+\hat{H}_{nn}+E^{BO}(\{{\textbf R}_{\textit I}\}) \Big) {\theta(\{\textbf{R}_{\textit I}\})} = E(\{{\textbf R_I}\}) {\theta(\{\textbf{R}_{\textit I}\})}.
\end{split}
\end{equation}

\noindent Here, $E^\textit{BO}{(\{\textbf{R}_{\textit I}\})}$ is the energy of this electron system, which is also ${\textbf R_I}$-dependent because electrons wavefunction indirectly depends on nuclei positions. The $\hat{H}_{nn1}$,
$\hat{H}_{nn2}$, and $\hat{H}_{nn3}$ in \textbf{Eq. \ref{ise}} are given as

\begin{equation}\begin{split}
 &  \hat{H}_{nn1} = - \sumi<I> \frac{{{\nabla}_{\textit{I}}^{2}}}{2 M_\textit{I}}   \\
 &  \hat{H}_{nn2} = - \sumi<I> \frac{1}{M_\textit{I}} {\int {\Psi'(\{\textbf{r}_{\textit{i}}\}; \{\textbf{R}_{\textit I}\})}^{\ast} {{\nabla}_{\textit{I}}} {{\Psi'(\{\textbf{r}_{\textit{i}} \}; \{\textbf{R}_{\textit I}\})}} d {\textbf r }} {{\nabla}_{\textit{I}}}  \\
 &  \hat{H}_{nn3} = - \sumi<I> \frac{1}{M_\textit{I}} {\int {{\Psi'(\{\textbf{r}_{\textit{i}}\}; \{\textbf{R}_{\textit I}\})}}^{\ast} {{\nabla}_{\textit{I}}^{2}} {{\Psi'(\{\textbf{r}_{\textit{i}} \}; \{\textbf{R}_{\textit I}\})}} d {\textbf r }}. 
\end{split}\end{equation}
...
($H_{nn2}$ and $H_{nn3}$ are ignored in the second line in \textbf{Eq. \ref{ise}} normally).
...
From here and following text, the $\textbf R$ is suppressed in the wavefunction since they are in fixed positions. The total energy of the electronic system can be written as

\begin{equation}\label{teh}
E^{\textit{H}} = <\wfh |\ {\hath}^{\textit{BO}} \ | \wfh  >.
\end{equation}

${\hath}^{\textit{BO}}$ can be rewritten as
\begin{equation}\begin{split}
&  {\hath}^{\textit{BO}} = \sum_i ( \frac{{{\nabla}_{\textit{i}}^{2}}}{2} + \sum_J \frac{Z_\textit{I}}{|\textbf{r}_\textit{i}-\textbf{R}_\textit{I}|}) +   \frac{1}{2} \sum_i \sum_{j \neq i} \frac{1}{ |\textbf{r}_\textit{i}-\textbf{r}_\textit{j}|} \\
& = \sum_i h_1(\textbf{r}_\textit{i}) + \frac{1}{2} \sum_i \sum_{j \neq i} h_2(\textbf{r}_\textit{i},\textbf{r}_\textit{j}).
\end{split}
\end{equation}
...
\begin{equation}\begin{split}
& E^\textit{H} = \sumi<i> <\bwff<i><i> | \  h_1(\textbf{r}_\textit{i}) \ | \bwff<i><i> > \\
& +\frac{1}{2} \sum_i \sum_{j \neq i} <\bwff<i><i> \bwff<j><j>|\ h_2(\textbf{r}_\textit{i},\textbf{r}_\textit{j}) \ | \bwff<i><i> \bwff<j><j>>.  
\end{split}\end{equation}

In order to calculate the stationary state with the lowest energy of the system, the variation of any wavefunction $\phi_{k}(\textbf{r}'')$ should be
zero variation in the energy. That is, 
\begin{equation}
 \phi_{k}(\textbf{r}'') \rightarrow \phi_{k}(\textbf{r}'') + \delta \phi_{k}(\textbf{r}'')  \Rightarrow \delta E^{\textit{H}} = 0.
\end{equation}

Furthermore, the constraint is that the wavefunctions need to be orthonormalized. According to method of Lagrange multipliers, the variation with  Lagrange multiplier $E_{i,j}^{H}$ is satisfied

\begin{equation}\label{haa}
 \delta \Big(E^{\textit{H}} - \sum_i \sum_j E_{i,j}^{H} (<\bwff<i><i> | \bwff<j><j>> -\ \delta_{ij})\Big)  = 0. 
\end{equation}
