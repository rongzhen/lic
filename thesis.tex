...
The $p$-$n$ junction is the fundamental building block of solar cells. The single $p$-$n$ homojunction will be explored in this section.

We start from the seperate $n$-type material and $p$-type material at room temperature (assume that all the donor (acceptor) atoms are positively (negatively) ionized at room temperature). In \textbf{Fig. \ref{dopedmaterials}},
the left one shows the $n$-type and $p$-type materials. The $n$-type
material has many free negatively charged electrons which can move freely in the material, and there are numbers of positively charged immobile donor ions as well. Similarily, the $p$-type material has many free positively
...
On the right in \textbf{Fig. \ref{dopedmaterials}}, the corresponding Fermi levels are shown. The Fermi level (E\textsubscript{nf}) is closer to conduction band minimum for $n$-type material due to the many free 
negatively charged electrons. Conversely, the Fermi level of $p$-type material (E\textsubscript{pf}) is closer to the valence band maximum to the free positively charged holes.
...
\begin{figure}[H]
    \begin{center}
     ...
     \end{center}
    \caption{Left: Doped ($n$-type and $p$-type) materials in dark at room temperature. Right: Energy band diagram of seperated $p$-$n$ homojunction in dark at room temperature for two-level model. Assume that all the donor (acceptor) atoms are 
...
"build in" electric field which points from the $n$-type material to the $p$-type material. This can cause the drift of carriers in the opposite direction. The "build-in" electric field will force the electrons (holes) back into
...
\begin{figure}[H]
    \begin{center}
     ...
     \end{center}
    \caption{Left: The $p$-$n$ homojunction in dark at room temperature. Right: Energy band diagram of a $p$-$n$ homejunction at the equilibrium in dark at room temperature for two-level model. }      
    \label{pnjunction}
\end{figure}
...
to the $n$-type side (conventional current). Because the electrons from valence bands (VBs) goes to conduction bands (CBs), which can generate pairs of electron-hole. At the same time, the recombination of paired electron-hole ocurrs. The rate of generation 
...
will jump down from CBs to VBs again. However, the electron-hole pairs will be seperated in the $p$-$n$ junction region due to the "build-in" electric field, therefore, the current will be generated. Actually, the electron-hole pairs in the either n-type and p-type 
...
\caption{The $p$-$n$ homejunction under illumination with load at room temperature for two-level model.}
\label{illu}
\end{figure}
...
for them is relatively low for CIS and CGS. Therefore, CIS is possbile to be $n$-type material. However, CGS is not possible to be $n$-type under equilibrium conditions, because the low formation energy of
...
temperature with thickness around \SI{500} {\nm}. The most important part of the device is the $p$-type absorber layer: CIGS, which is dopped by intrinsic defects and around \SI{1500} -- \SI{2000} {\nm}. 
...
avoid the damage of the CIGS and CdS from sputtering damage when depositing the ZnO:Al window layer, and the $n$-type ZnO is dopped by the aluminum (Al) in order to get higher conductivity. 
...
Here, term with the superscript "$en$" implies that this term is related with electrons and nuclei. ${\Psi^{en}(\{ {\textbf r}_{i}, {\textbf R}_{I} \})}$ is the exact wavefunction, $\textbf{r}_{i}$ 
...
\textbf{Eq. \ref{ssth}} can not be solved exactly at present since there are enormous amount of atoms to calculate in reality. More importantly, the exactly form of the wavefunction is unknown.
...
operator term. The nuclei are treated as fixed because the mass of nucleus is much larger than that of electron. This indicates that the electrons are seen as interacting under both the external potential caused by nuclei that are fixed in 
...
& {H^n} = - \sum\limits_I \frac{{{\nabla}_{{I}}^{2}}}{2 M{I}} + {{U}_{zz}}.
 \end{split}
\end{equation}
...
& U_{ext} = - \sum\limits_i^{{Ne}} \sum\limits_J^{{Nn}} \frac{Z_{I}}{|\textbf{r}_{i}-\textbf{R}_{I}|} \\
& {U}_{int} =  \frac{1}{2} \sum\limits_i^{{Ne}} \sum\limits_{j \neq i}^{{Ne}} \frac{1}{ |\textbf{r}_{i}-\textbf{r}_{j}|} \\
& {{U}_{zz}} = \frac{1}{2} \sum\limits_I^{{Nn}} \sum\limits_{J \neq I}^{{Nn}} \frac{Z_{I} Z_{J}\ }{|\textbf{R}_{I}-\textbf{R}_{J}|}.
\end{split}
\end{equation}
...
& \Big( {U}_{zz1}+{U}_{zz2}+{U}_{zz3}+{U}_{zz}+E(\{{\textbf R}_{I}\}) \Big) {\theta(\{\textbf{R}_{I}\})} = E^{en}(\{{\textbf R}_I\}) {\theta(\{\textbf{R}_{I}\})}.
\end{split}
\end{equation}

\noindent Here, $E{(\{\textbf{R}_{I}\})}$ is the total energy of electronic system, which is also ${\textbf R}_I$-dependent because electrons wavefunction indirectly depends on nuclei positions. The ${U}_{zz1}$,
${U}_{zz2}$, and ${U}_{zz3}$ in \textbf{Eq. \ref{ise}} are derived

\begin{equation}\begin{split}
 &  {U}_{zz1} = - \sum\limits_I^{{Nn}} \frac{{{\nabla}_{{I}}^{2}}}{2 M_{I}}   \\
 &  {U}_{zz2} = - \sum\limits_I^{{Nn}} \frac{1}{M_{I}} {\int {\Psi'(\{\textbf{r}_{{i}}\}; \{\textbf{R}_{I}\})}^{\ast} {{\nabla}_{{I}}} {{\Psi'(\{\textbf{r}_{{i}} \}; \{\textbf{R}_{I}\})}} d {\textbf r }} {{\nabla}_{{I}}}  \\
 &  {U}_{zz3} = - \sum\limits_I^{{Nn}} \frac{1}{M_{I}} {\int {{\Psi'(\{\textbf{r}_{{i}}\}; \{\textbf{R}_{I}\})}}^{\ast} {{\nabla}_{{I}}^{2}} {{\Psi'(\{\textbf{r}_{{i}} \}; \{\textbf{R}_{I}\})}} d {\textbf r }}. 
\end{split}\end{equation}
...
($U_{zz2}$ and $U_{zz3}$ are ignored in the second line in \textbf{Eq. \ref{ise}} normally).
...
electrons. The wavefunction with $Ne$ independent electrons is given as
...
In order to calculate the stationary state with the lowest energy of the system, the method of Lagrange multipliers is ultilized. Furthermore, the constraint is that the different states of electrons are orthonormalized.
...
Here, $ \delta \phi_{k}(\textbf{r})$ also can be utilized, however, variation with respect to $\phi^*_{k}(\textbf{r})$ and $\phi_{k}(\textbf{r})$ are equivalent. The result is more organized using $\phi^*_{k}(\textbf{r})$.

In comparison with \textbf{Eq. \ref{bzd}}, $\delta E^{{H}}$ in \textbf{Eq. \ref{haa}} can be calculated by two parts. The first part is 
...
Here, the factor of $\frac{1}{2}$ cancels because the $2$nd ($3$rd) line is the same with $4$th ($5$th) line in \textbf{Eq. \ref{hartree2}} due to the exchangeable of index of $i$ and $j$. 
...
Schrödinger equation is described in a matrix determinant for the $N$ number of electrons (for aesthetical reason, $N$ implies the number of electrons and not $Ne$ as in previous sections)
...
& \{ -\frac{\nabla^{2}_{i}}{2} + \sum\limits_I^{{Nn}} \frac{Z_{I}}{|\textbf{r}-\textbf{R}_{I}|} + \sum\limits_{j}^{{N}} <\bwfn<j><'>\ | \frac{1}{|\textbf{r} - \textbf{r}^{'} |} \ | \bwfn<j><'>> \} \bwff<i><>  \\
...
\{ -\frac{\nabla^{2}_{i}}{2} +  \sum\limits_I^{{Nn}} \frac{Z_{I}}{|\textbf{r}-\textbf{R}_{I}|} +V_{HF}(\textbf{r}) \}  \bwff<i><>  = \epsilon_{i}^{{HF}}   \bwff<i><>.
\end{split}\end{equation}

\begin{equation}
\begin{split}
&  V_{HF}(\textbf{r})= \int \frac{\rho(\textbf{r}{'}) - \rho_{{i}}^{{HF}}(\textbf{r},\textbf{r}{'})}{|\textbf{r} - \textbf{r}{'} |}  \mathrm{d}\textbf{r}{'} \\
...
The proof of the theorem is given. Let us assume that there exsits two external potentials named $V_{ext}(\textbf{r})$ and $V'_{ext}(\textbf{r})$ leading to the same ground state 
...
\begin{equation}\label{hkpf44}
  E'  < \  E + \int d \textbf{r} \Big( V'_{ext}(\textbf{r}) - V_{ext}(\textbf{r}) \Big)  \rho(\textbf{r}).
\end{equation}
...
\begin{equation}\begin{split}
 & E[\rho'] \ =   \ \int \rho'(\textbf{r}) V_{ext}  + \  F[\rho'] > E[\rho]. 
\end{split}
\end{equation}
 
Here, the total energy for the case of exact ground-state electron density is lower than any other cases. Therefore, the exact ground-state electron density by minimizing the total energy can be achived.

From those two theorems, one knows how to solve Schrödinger equation theoretically. However in practice, $E[\rho]$ is unknown. Therefore, another method is needed to solve it, which is so called Kohn-Sham (KS) equation.
...
Assume that the exact ground-state density is obtained by the Hartree-like wavefunction $\wfks = \bwf<1><1> \bwf<2><2> \cdots \bwf<N><N>$. Here, $N$ is a number of electrons and $\bwf<i><i>$ is auxiliary independent single-electron wavefuntion,
and electron density is defined as
...
& V_{H}[\rho] \ = \ \frac{1}{2} \int \int \mathrm{d} {\textbf{r}} \mathrm{d}{\textbf{r}'} \frac{\rho({\textbf{r}})\rho(\textbf{r}')}{|{\textbf{r}}-{\textbf{r}}'|} \\
& V_{ext}[\rho]\ = \ \int \mathrm{d}{\textbf{r}} \rho(\textbf{r}) V_{ext}(\textbf{r}). 
\end{split}
\end{equation}
...
In \textbf{k}-space, Kohn-Sham equation can be written as
\begin{equation}\label{aaa111}
 \Big(-\frac{\nabla^{2}}{2}\ + \ V^{KS}(\textbf{r})\Big) \Psi^{{KS}}_{{i,k}}(\textbf{r}) = \epsilon^{{KS}}_{{i,k}} \Psi^{{KS}}_{{i,k}}(\textbf{r}).
\end{equation}


The total energy is not given in \textbf{Eq. \ref{aaa}}. However, if one changes the \textbf{Eq. \ref{aaa}} as 
...
So far there are two problems still in the air: one is the exact format of $V_{xc}(\textbf{r})$; the other one is how to solve the Kohn-Sham equation.
...
Here, $r_s = (3 / (4 \pi \rho))^{1/3}$. In the LDA, the idea is that the exchange-correlation energy for an electron in the very tiny small volume in many-particle system is equal to the exchange-correlation for an elecetron 
...
Today, there exists hundreds of different exchange-correlation potentials. It is stll ongoing development. The advangtage of Kohn-Sham equation is that new potentials are implemented easily. In the same time, the number of potential is huge due to
the simplicity of implementation.

\subsection{Solving the secular equation}

The process for solving Kohn-Sham equation can be achived by iteration. Because potentials in Kohn-Sham equation depends on electron density. However, electron density is calculated by wavefunction which depends on potential. 
An initial guess of electron density is defined at the beginning of the calculation, then the equation is solved iteratively until the reasonable solution is obtained.
...
\begin{equation}\label{ep2}
 \hath \Psi^{KS} (\textbf{r}) = E \Psi^{KS} (\textbf{r}).
\end{equation}
...
If \textbf{Eq. \ref{ep1}} is plugged into \textbf{Eq. \ref{ep2}}, and left multiply $\Phi^*_1, \Phi^*_2, \cdot, \Phi^*_N$ one by one, it ends up with a set of equations
...
\\
\sum\limits_{\alpha}^{N_{\alpha}} \sum\limits_{{\ell}}^{N_{\ell}} \sum\limits_{{m}}^{N_{m}} f_{{\ell}{m}} (r_{\alpha},{\textbf {k}+\textbf{G}},\epsilon_{i,k}^{KS}) Y_{{\ell}m}(\widehat{{\textbf r}}_{\alpha})  & \quad \mbox{if $ r_{\alpha} \in MT. $}\\ 
\end{cases}
\end{equation}

Here, $f_{{\ell}{m}} (r_{\alpha},\textbf{k}+\textbf{G},\epsilon_{i,k}^{KS}) =  A _{{\ell}m}^{\alpha} ( \textbf {k}+\textbf{G}) u_{{\ell}}(r_{\alpha},\epsilon_{i,k}^{KS})$, and $A _{{\ell}m}^{\alpha} (\textbf {k}+\textbf{G}) $ is the 
expansion coefficients. Radial function $u_{{\ell}} (r_{\alpha},\epsilon_{i,k}^{KS})$ is dependent with energy $\epsilon_{i,k}^{KS}$, which is the eigenvalue in \textbf{Eq. \ref{aaa111}}. The radial function can be decided by 

\begin{equation}\label{ap2}
-\frac{1}{r^2}  \frac{d}{dr} (r^2 \frac{du_{\ell}}{dr})+ \left(\frac{\ell(\ell+1)}{r^2}+V^{KS}_0({r})\right)u_{\ell}(r_{\alpha},\epsilon_{i,k}^{KS}) = \epsilon_{i,k}^{KS} u_{\ell}(r_{\alpha},\epsilon_{i,k}^{KS}).
\end{equation}
...
\begin{equation}
A _{{\ell}m}^{\alpha} (\textbf {k}+\textbf{G})  u_{{\ell}}^{\alpha}(r_{\alpha},\epsilon_{i,k}^{KS})=  \frac{4 \pi }{\sqrt{\Omega}} e^{i (\textbf {k}+\textbf{G}) \textbf{R}_{\alpha} }  \sum\limits_{{\ell}}^{N_{\ell}} \sum\limits_{{m}}^{N_{m}} i^{\ell} \bessf< |\textbf{k}+\textbf{G}|> Y^{*}_{\ell{m}}(\widehat{\textbf{k}+\textbf{G}}).
\end{equation}

The main drawbacks about the APW method is that the wavefuntion is energy dependent (\textbf{Eq. \ref{ap2}}). It generates that
...
Here, eigenvalue is written without subscript and superscript $\epsilon$. Therefore the basis fucntion is re-defined as 
...
. $A _{{\ell}m}^{\alpha} (\textbf {k}+\textbf{G})$ and $B _{{\ell}m}^{\alpha} (\textbf {k}+\textbf{G})$ are the expansion coefficients, and $\dot{u}_{{\ell}}(r_{\alpha}, \epsilon_{\ell,\alpha} )$ is the derivative of the radial function. Energy $\epsilon_{\ell,\alpha}$  is considered as pre-calculated parameter in \textbf{Eq. \ref{lap4}}. Actually, it is chosen by the middle of  each $\ell$-character band, therefore this method is called linearized augmented plane wave (LAPW) method.
...
Here, $f$ is the Fermi distribution function. $c$ and $v$ are the index of conduction band and valence band, respectively. The total imaginary part of dielectric function can be calculated when $c$ and $v$ run over all the conduction bands and valence bands. Similarily, the
...
Here, $\ubloch<n><\bold k>$ is a function which has the same a periodicity as the potential. If substituting \textbf{Eq. \ref{1}} to \textbf{Eq. \ref{kpse}}, the new equation is derive as
...
Here, $H_0$ can be seen as unpertubation terms, and remaining terms are seen as pertubation terms. From the pertubation theory, the following equation can be obtained  
...
In the realistic implmentation, the following derivation is exploited. Assume that the wavefunction and eigenvalue are obtained by some procedures on $\textbf{k}_0$ point. $\wfbloch<n><\bold{k}_0>$ is the wavefunction and 
$\epsilon_{n,\textbf{k}_0}$ is the eigenvalue. Luttinger-Kohn functions are given as
...
& {\sum\limits_{j}^{N}}  C_{n,j}^{\bold k} \left \{  \left [  \epsilon_{n,\textbf{k}_0} -  \epsilon_{n,\textbf{k}}  + \frac{{\hbar}^2}{2m} { (\bold {k}^2- \bold {k}_0^2)}    \right ] \delta_{j',j} + \frac{\hbar}{m} {(\bold {k}-\bold{k}_0)} {\overline{\textbf{p}}_{j',j}} \right \} = 0 \\
...
& H_{j',j} = \left [   \epsilon_{n,\textbf{k}_0}  + \frac{{\hbar}^2}{2m} { (\bold{k}^2-\bold{k}_0^2)}    \right ] \delta_{j',j} + \frac{\hbar}{m} {(\bold{k}-\bold{k}_0)} {\overline{\textbf{p}}_{j',j}}.
\end{split}
\end{equation}

Here, the coefficient $ C_{n,j}^{\bold k}$ and $\epsilon_{n,\textbf{k}}$ can be calculated. Therefore, the wavefunction on ${ \textbf k}$ point can be eobtained.
...
{H}_{dirac} = c \boldsymbol{\alpha} \textbf{P} + \boldsymbol{\beta}mc^{2} + V.
\end{equation}
...
\Big(V + \frac{1}{2m} (\boldsymbol{\sigma} \textbf{P}) (1+\frac{E^{\prime}-V}{2mc^2})^{-1} (\boldsymbol{\sigma} \textbf{P}) \Big)\phi=E^{\prime}\phi,
\end{equation}
...
\textbf{Eq. \ref{nonrela1}} is only valid if the velocity of particles is smaller than the one of light. It is not true in the region which is close to the nucleus, especially for heavy elements, where the relativistic effects
are relatively strong. Moreover, the Coulomb potential can be arbitrarily large negative energies in realistic situation as well, which has the singularity of the potential at the nucleus. Therefore, there is another way to derive 
...
& \Big(V + (\boldsymbol{\sigma} \textbf{P}) \Big(\frac{c^2}{2mc^2-V}\Big)\Big(1+\frac{E^{\prime}}{2mc^2-V}\Big)^{-1} (\boldsymbol{\sigma} \textbf{P}) \Big)\phi \\
& \approx \Big( V + (\boldsymbol{\sigma} \textbf{P}) \Big(\frac{c^2}{2mc^2-V}\Big)(\boldsymbol{\sigma} \textbf{P}) \Big)\phi \\
& - (\boldsymbol{\sigma} \textbf{P}) \Big(\frac{c^2}{2mc^2-V}\Big)\Big(\frac{E^{\prime}}{2mc^2-V}\Big)^{-1} (\boldsymbol{\sigma} \textbf{P}) \Big)\phi + \cdot \\
& =E^{\prime}\phi.
\end{split}
\end{equation}

Here, the Taylor expansion is utilized. The zeroth order regular approxiation (ZORA) hamiltonian is definded as
\begin{equation}\begin{split}\label{zora1}
& H^{zora} = V + (\boldsymbol{\sigma} \textbf{P}) \Big(\frac{c^2}{2mc^2-V}\Big)(\boldsymbol{\sigma} \textbf{P}) \\
& =  V + \textbf{P} \Big(\frac{c^2}{2mc^2-V}\Big)\textbf{P} + \frac{c^2}{(2mc^2-V)^2} \boldsymbol{\sigma}\cdot(\bigtriangledown V\times \textbf{P})
 \end{split}
\end{equation}
...
Since as-grown CuIn\textsubscript{1-x}Ga\textsubscript{x}Se\textsubscript{2} typically has $p$-type character, the Fermi level and $p$-type carrier concentration of free holes $n_v(T)$ and free electrons $n_c(T)$ is calculated by 
...
( $T > 400 K$) and for low acceptor concentrations, the Fermi level of $p$-type CuIn\textsubscript{1-x}Ga\textsubscript{x}Se\textsubscript{2} will reach the Fermi level of corresponding intrinsic compounds. Moreover, 
