...
The eigenfunctions of this Hamiltonian can be determined by the eigenstates of the total angular momentum (\textbf{J} = \textbf{L} + \textbf{S}) and $z$ component of $J$ ($J_z$). In the case of $L$ = 1 and S=$\frac{1}{2}$,
...
Typically, the low conduction bands are with $\varGamma_1$ and $\varGamma_4$ symmetries. Therefore, there are 14 unperturbed wavefunctions considering both the valence and conduction bands. However, the 6 $\varGamma_4$ valence bands are of 
...
Here, $H_{ij}$ is the remaining terms in \textbf{Eq. \ref{kp1}}, and the 6$\times$6 matrix \{$H_{ij}^{'}$\} is a Hermitian matrix.

Taking advantage of valence band wavefunctions (\textbf{Eq. \ref{99}}) and conduction band wavefunctions, the 6$\times$6 matrix \{$H_{ij}^{'}$\} can be calculated. Assume that $k$ is so small that $H_{15}^{'}$, $H_{16}^{'}$,
...
& \epsilon_{hh} = -Ak^2-[B^2k^4 + C^2 (k_x^{2}k_y^{2} + k_y^{2}k_z^{2} +k_z^{2}k_x^{2} )]^{\frac{1}{2}} \\
& \epsilon_{lh} = -Ak^2+[B^2k^4 + C^2 (k_x^{2}k_y^{2} + k_y^{2}k_z^{2} +k_z^{2}k_x^{2} )]^{\frac{1}{2}}.
 \end{split}
\end{equation}
...
\begin{equation}\begin{split}\label{1010}
 ...
 & B = \frac{\hbar^2}{3m_e} \Big\{ \frac{-P^2}{m_e{\epsilon_0}} + \frac{Q^2}{m_e{\epsilon_0^{'}}} \Big\} \\
 & C^2 = \frac{4\hbar^2P^2Q^2}{3m_e^4\epsilon_0\epsilon_0^{'}}.
 \end{split}
\end{equation}

Assume that the valence band masses are isotropic, the average heavy hole mass ($m_{hh}$) and light hole mass ($m_{lh}$) can be calculated from  \textbf{Eq. \ref{8652}} 
...
To utilize the $\bold k \cdot \bold p$ method for a concrete example, band dispersions of the valence band edges of silicon are derived briefly.
Spin-orbit interaction is considered in the derivation. 

Valence band edge occurs at \textbf{k}=0 in the Brillouin zone.  Wavefunctions of these valence bands are \textit{p}-like. 
