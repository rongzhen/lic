...
To utilize the $\bold k \cdot \bold p$ method, top valence band dispersions near a degenerate extremum are derived briefly for zinc blende type semiconductors. Spin-orbit interaction is considered. The detailed 
derivation can be found in Refs. \cite{fospamp2010, suzuki1995first, dresselhaus1955cyclotron, persson1997relativistic}.


The top valence bands are with $\varGamma_4$ symmetry for zin blende structure .Wavefunctions of these valence bands are \textit{p}-like which can be represented by the eigenstates $|X>$,$|Y>$, and $|Z>$. 
The orbital angular momentum $l$ is equal to 1 and have three degenerate states for \textit{p}-like wavefunction. The three degenerate states can be determined from magnetic quantum numbers $m_l$ (1, 0, and $-$1).

\begin{equation}\begin{split}\label{77}
& | 1, 1 > = - \frac{(|X>+i|Y>)}{\sqrt{2}} \\
& | 1, 0 > =  |Z> \\
& | 1, -1 > = - \frac{(|X>-i|Y>)}{\sqrt{2}}.
\end{split}
\end{equation}
...
The eigenfunctions of this Hamiltonian can be determined by the eigenstates of the total angular momentum (\textbf{j} = \textbf{l} + \textbf{s}) and $j_z$. In the case of $l$ = 1 and s=$\frac{1}{2}$,  $j$ is either equal to $\frac{3}{2}$
or $\frac{1}{2}$, and $j_z$ can be decided from values $j$, $j-1$, $\cdots$, $-j$. Therefore, the eigenfunctions of spin-orbit coupling Hamiltonian can be given as 

\begin{equation}\begin{split}\label{99}
& | \frac{3}{2}, \frac{3}{2} > = |1,1> \alpha \\
& | \frac{3}{2}, \frac{1}{2} > = \frac{|1,1> \beta + \sqrt{2} |1,0>\alpha}{\sqrt{3}} \\
& | \frac{3}{2}, -\frac{1}{2} > = \frac{|1,-1> \alpha + \sqrt{2} |1,0>\beta}{\sqrt{3}} \\
& | \frac{3}{2}, -\frac{3}{2} > = |1,-1> \beta \\
& | \frac{1}{2}, \frac{1}{2} > = \frac{|1,0> \alpha - \sqrt{2} |1,1>\beta}{\sqrt{3}} \\
& | \frac{1}{2}, -\frac{1}{2} > = \frac{|1,0> \beta - \sqrt{2} |1,-1>\alpha}{\sqrt{3}}. 
\end{split}
\end{equation}
...
Typically, the low conduction bands are with $\varGamma_1$ and $\varGamma_4$ symmetries. There are 14 unperturbed wavefunctions considering the valence and conduction bands. However, the 6 $\varGamma$ valence bands are of 
interest. The interaction between valence bands and conduction bands is considered as pertubation, the effective matrix element in 6$\times$6 matrix is defined as

\begin{equation} 
 H_{ij}^{'} = H_{ij}+\sum\limits_{k \neq valence bands} \frac{H_{ik}H_{kj}}{E_i-E_k}.
\end{equation}


Taking advantage of valence band wavefunctions (\textbf{Eq. \ref{99}}) and conduction band wavefunctions, the matrix {$H_{ij}^{'}$} can be calculated. Assume that $k$ is so small enough that $H_{15}^{'}$, $H_{16}^{'}$,
...
\begin{equation}
 E_{v3} = -\varDelta_{SO} + \frac{\hbar^2 k^2}{2m_0} \{ 1 - \frac{2}{3} (\frac{P^2}{m(E_0 + \varDelta_{SO})} + \frac{2Q^2}{m(E_0^{'} + \varDelta_{SO})})  \}.
\end{equation}

Here, $\varDelta_{SO}$ is the spin-orbit splitting energy of the valence band. $E_0$ is the energy difference between the valence bands with $j$=$\frac{3}{2}$ and the conduction bands with symmetry $\varGamma_1$. 
 $E_0$ is the energy difference between the valence bands with $j$=$\frac{3}{2}$ and the conduction bands with symmetry $\varGamma_4$. 

The energy dispersion of the $j$ = $\frac{3}{2}$ can be calculated by diagonalizeing the 4$\times$4 matrix, which is derived as

\begin{equation} \begin{split}
& E_{v1} = -AK^2+[B^2k^4 + C^2 (k_x^{2}k_y^{2} + k_y^{2}k_z^{2} +k_z^{2}k_x^{2} )] \\
& E_{v2} = -AK^2-[B^2k^4 + C^2 (k_x^{2}k_y^{2} + k_y^{2}k_z^{2} +k_z^{2}k_x^{2} )]
 \end{split}
\end{equation}
...
\begin{equation}\begin{split}\label{1010}
 & A = \frac{\hbar^2}{2m} \{ 1 - \frac{2}{3}(\frac{P^2}{m{E_0}} + \frac{2Q^2}{m{E_0^{'}}}) \} \\
 & B = \frac{\hbar^2}{3m} \{ \frac{-P^2}{m{E_0}} + \frac{2Q^2}{m{E_0^{'}}} \} \\
 & C^2 = \frac{8P^2Q^2}{3m^3E_0E_0^{'}}
 \end{split}
\end{equation}

The average effective hold masses in \textbf{Eq. \ref{1010}} can be calculated by 

\begin{equation}\begin{split}
&  m_{hh}^{-1} = \frac{1}{\hbar^2} [ -2A + 2B (1+ \frac{2|C|^2}{15B^2})] \\
&  m_{lh}^{-1} = \frac{1}{\hbar^2} [ -2A - 2B (1+ \frac{2|C|^2}{15B^2})]
 \end{split}
\end{equation}


The top valence band dispersion and effective masses can be determined if the constants A, B, and C are known. In realistic application, they are the fitting parameters. 
