...
In the realistic implmentation, the following derivation is exploited. Assume that the wavefunction and eigenvalue are obtained by some procedures on $\bold k_0$ point. $\wfbloch<n><\bold  k_0>$ is the wavefunction and 
$\epsilon_{n,\textbf{0}}$ is the eigenvalue. Luttinger-Kohn functions are given as
...
In \textbf{Eq. \ref{kp3}}, $N$ can be $\infty$. The wavefunction can be obtained if the coefficient $C_{n,j}^{\bold k}$ is obtained. Based on \textbf{Eq. \ref{kp3}} and  \textbf{Eq. \ref{kpse}}, the following equation is derived
Finally the following equation is obtained:

\begin{equation}\label{5}
{\sum\limits_{j}^{N}}  C_{n,j}^{\bold k} \left \{  \left [  \ebloch<j><\bold {k_0}> -  \ebloch<n><\bold k>  + \frac{{\hbar}^2}{2m} { (\bold {k}^2- \bold {k}_0^2)}    \right ] \delta_{j',j} + \frac{\hbar}{m} {(\bold {k}-\bold{k}_0)} {\overline{\textbf{p}}_{j',j}} \right \} = 0,
\end{equation}
