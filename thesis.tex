...
For most of zinc blende type semiconductors, valence bands of the valence band edges have 3-fold orbital degeneracy (6 if one considers spin) at $\Gamma$ point in the Brillouin zone without considering spin-orbit interaction.
They have 6-fold orbital degeneracy considering spin-orbit interaction.

\begin{figure}[H]
\centering
%\includegraphics[scale=0.5]{devicestruc.eps} 
\includegraphics[scale=0.8]{bands.eps} 
\caption{Band degeneracy near the valence band edges with and without spin-orbit coupling (SOC) for most of zinc blende type semiconductors.}
\label{device}
\end{figure}
...
of the $J$ = $\frac{3}{2}$ can be calculated by diagonalizing the 4$\times$4 matrix, which is derived as
...
The average heavy hole mass ($m_{hh}$) and light hole mass ($m_{lh}$) can be calculated from  \textbf{Eq. \ref{8652}} 
...
The 2$\times$2 matrix can be treated as the case of degenerate $J$ = $\frac{1}{2}$, and the energy dispersion can be derived as

\begin{equation}
 \epsilon_{SO} (\textbf{k}) = -\varDelta_{SO} + A \frac{\hbar^2 {k}^2}{2m_0}.
\end{equation}

Here, $\varDelta_{SO}$ is the spin-orbit splitting energy of the valence band.

The average effective mass $m_{SO}$ can be calculated from  \textbf{Eq. \ref{8652}} 
...
\begin{comment}
To utilize the $\bold k \cdot \bold p$ method for a concrete example, band dispersions of the valence band edges of silicon (diamond strucutre) are derived briefly.
Spin-orbit interaction is considered in the derivation. 

Valence band edge occurs at \textbf{k}=0 in the Brillouin zone for silicon.  Wavefunctions of these valence bands are \textit{p}-like. 
The \textit{p}-like wavefunction has three degenerate states if the spin-orbit interaction is not considered, which can be represented by the eigenstates $|X\rangle$, $|Y\rangle$, and $|Z\rangle$.
The \textit{p}-like wavefunction corresponds to the orbital angular momentum is equal to 1. The three degenerate states can be expressed from the orbital angular momentum (1) and magnetic quantum numbers (1, 0, and $-$1) by
...
\begin{equation} \begin{split}\label{8652}
\epsilon_{v1, v2} = Ak^2\pm[B^2k^4 + C^2 (k_x^{2}k_y^{2} + k_y^{2}k_z^{2} +k_z^{2}k_x^{2} )]^{\frac{1}{2}}.
 \end{split}
\end{equation}
...
\begin{equation}\begin{split}\label{1010}
 & A = \frac{\hbar^2}{2m_e} + \frac{1}{3}(L+2M) \\
 & B = \frac{1}{3}(L-M) \\
 & C^2 = \frac{1}{3}[N^2-(L-M)^2].
 \end{split}
\end{equation}

Here, $L$, $M$, and $N$ are electron momentum matrix elements. Actually, they are not important in the realistic application because only  $A$, $B$, and $C$ are the fitting parameters.

The 2$\times$2 matrix can be treated as the case of degenerate $J$ = $\frac{1}{2}$, and the energy dispersion can be derived as

\begin{equation}
 \epsilon_{SO} = -\varDelta_{SO} + Ak^2.
\end{equation}

Here, $\varDelta_{SO}$ is the spin-orbit splitting energy of the valence band.


From the band dispersion, effective masses and many other properties can be obtained. The derivation is brief, and the detailed derivation can be found in Refs. \cite{fospamp2010, suzuki1995first, dresselhaus1955cyclotron, persson1997relativistic}.
\end{comment}
...
Valence band edge occurs at \textbf{k}=0 in the Brillouin zone for silicon.  Wavefunctions of these valence bands are \textit{p}-like. 
The \textit{p}-like wavefunction has three degenerate states if the spin-orbit interaction is not considered.

The arbitrary choice of a basis for the representation $\varGamma_{25}^{+}$ at \textbf{k}=0, and Hamiltonian $\hbar\textbf{kp}/m_e$ is considered as perturbation. The following perturbation matrix is derived without consideration 
...
Here, $L$, $M$, and $N$ are electron momentum matrix elements. The energy dispersion can be obtained from $\lambda$ by
...
The energy dispersion can be calculated by diagonalizing the 4$\times$4 matrix, which is derived as
...
The energy dispersion can be derived by diagonalizing the 2$\times$2 matrix, which is derived as
