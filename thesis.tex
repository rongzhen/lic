...
where $\wf$  is the exact wavefunction, $\textbf{r}_\textit{i}$ and $\textbf{R}_\textit{I}$  stands for coordinators of electron and
...
where the indices $\textit{i}$, $\textit{j}$ are used for the electrons and $\textit{I}$, $\textit{J}$ are used for atomic nuclei, $Z_\textit{I}$ means the charge of the $\textit{I}$-th nucleus,
...
The \textbf{Eq. \ref{sth}} can be re-defined as

\begin{equation}\begin{split}\label{nh}
& {\hath} = - \sumi<I> \frac{{{\nabla}_{\textit{I}}^{2}}}{2 M_\textit{I}} + \frac{1}{2} \suminj<I><J> \frac{Z_\textit{I} Z_\textit{J}\ }{|\textbf{R}_\textit{I}-\textbf{R}_\textit{J}|} + {\hath}^{BO} \\
...
&{\hath}^\textit{BO}\ =\ - \sumi<i>   \frac{{{\nabla}_{\textit{i}}^{2}}}{2}  - \sumi<i> V_{ext}(\textbf{r}_{\textit{i}})  + \frac{1}{2} \suminj<i><j> \frac{1}{ |\textbf{r}_\textit{i}-\textbf{r}_\textit{j}|} \\
&\ = \ \hat{T} \ + \ \hat{V}_{ext} \ + \ \hat{V}_\textit{int}\ ,
\end{split}\end{equation}
...
where ${\hath}^\textit{BO}$  is the Hamitonian for the electrons within the Born-Oppenheimer approximation. The subscript $ext$ means $external$ in \textbf{Eq. \ref{both}}, and $V_{ext}(\textbf{r}_{\textit{i}})$ describes the external potentials interaction. 

Thereby, the new Schrödinger equation combined with \textbf{Eq. \ref{nwf}} and \textbf{Eq. \ref{nh}} is 

\begin{equation}
 (- \sumi<I> \frac{{{\nabla}_{\textit{I}}^{2}}}{2 M_\textit{I}} + \hat{H}_{I,J} + {\hath}^{BO} ) ({\theta(\{{\textbf R}_{\textit I}\})} {\Psi'(\{\textbf{r}_{\textit i}\}; \{{\textbf R}_{\textit I}\})}) = E(\{{\textbf R_I}\}) ({\theta(\{{\textbf R}_{\textit I}\})} {\Psi'(\{\textbf{r}_{\textit i}\}; \{{\textbf R}_{\textit I}\})}),
\end{equation}
 
\noindent where, $E(\{{\textbf R_I}\})$ is the total energy. One can end up with the following equation using \textbf{Eq. \ref{both}}
...
& ( \hat{H}_{I1}+\hat{H}_{I2}+\hat{H}_{I3}+\hat{H}_{I,J}+E^{BO}(\{{\textbf R}_{\textit I}\}) ) {\theta(\{\textbf{R}_{\textit I}\})} = E(\{{\textbf R_I}\}) {\theta(\{\textbf{R}_{\textit I}\})},
\end{split}
\end{equation}

\noindent where $E^\textit{BO}{(\{\textbf{R}_{\textit I}\})}$ is the energy of this electronic system, and 
...
In summary, the Schrödinger equation of the electrons and nuclei is derived seperately within the Born-Oppenheimer approximation. When one refers to calculations of the ground state properties,
which means the Schrödinger equation of the electrons is applied, e.g., first line in \textbf{Eq. \ref{ise}}, and Schrödinger equation of nuclei is used for the calculation of lattice dynamics.

The \textbf{Eq. \ref{ise}} (first line) is much simpler than \textbf{Eq. \ref{nwf}}. However, it is still not solvable. Further approximations  are needed
...
where $i$ goes through all the electrons, and $\bwff<i><i>$ means state of the $i$-th electron in the position of ${\textbf{r}}_{\textit{i}}$.
From here and following, the $\textbf R$ is suppressed in the wavefunction since they are in fixed position. The total energy of the system can be written as

\begin{equation}\label{teh}
E^{\textit{H}} = <\wfh |\ {\hath}^{\textit{BO}} \ | \wfh  >.
\end{equation}

Therefore, the total energy of system is given by making the substitution using \textbf{Eq. \ref{both}} and \textbf{Eq. \ref{wfh}} into \textbf{Eq. \ref{teh}}
\begin{equation}\begin{split}
...
where $\textbf{r}_\textbf{i}$ and $\textbf{r}_\textbf{j}$ are written in generalized format $\textbf{r}$ and $\textbf{r'}$, respectively.
...
where $(<\bwff<i><> | \bwff<i><> > -1)$ means that the wavefunctions are orthogonal.
...
&  \delta ( \sumi<i> <\bwff<i><> |\ -\frac{\nabla^{2}_{ i}}{2} + V_\textit{ext}(\textbf{r}_\textbf{i}))  \ | \bwff<i><> > )  \\
...
& =  \frac{1}{2} \sum_{i} ( <\bwff<i><> \delta \phi_{k}(\textbf{r}'') |\ \frac{1}{|\textbf{r} - \textbf{r}'' |} \ | \bwff<i><> \phi_{k}(\textbf{r}'')> \\
& + <\bwff<i><> \phi_{k}(\textbf{r}'') |\ \frac{1}{|\textbf{r} - \textbf{r}'' |} \ | \bwff<i><> \delta \phi_{k}(\textbf{r}'')>) \\
& + \frac{1}{2} \sum_{j} ( \delta \phi_{k}(\textbf{r}'') \phi_{j}(\textbf{r}') |\ \frac{1}{|\textbf{r}'' - \textbf{r}' |} \ | \phi_{k}(\textbf{r}'') \phi_{j}(\textbf{r}') \\
& + \phi_{k}(\textbf{r}'') \phi_{j}(\textbf{r}') |\ \frac{1}{|\textbf{r}'' - \textbf{r}' |} \ | \delta \phi_{k}(\textbf{r}'') \phi_{j}(\textbf{r}') ) \\
& =  \sum_{i} ( <\bwff<i><> \delta \phi_{k}(\textbf{r}'') |\ \frac{1}{|\textbf{r} - \textbf{r}'' |} \ | \bwff<i><> \phi_{k}(\textbf{r}'')> \\
& + <\bwff<i><> \phi_{k}(\textbf{r}'') |\ \frac{1}{|\textbf{r} - \textbf{r}'' |} \ | \bwff<i><> \delta \phi_{k}(\textbf{r}'')>), \\
\end{split}\end{equation}

where factor of $\frac{1}{2}$ cancels because the \textit{2}nd (\textit{3}rd) line is the same with \textit{4}th (\textit{5}th) line in \textbf{Eq. \ref{hartree2}} due to the symmetry of $r$ and $r'$, as well as $i$ and $j$. 

To get the final solution, one more calculation is given as 

\begin{equation} \begin{split}\label{xixihaha}
& \delta \sumi<i> E_{i}^{H} (<\phi_{i}(\textbf{r}) | \phi_{i}(\textbf{r}) > -1) \\
& =  E_{k}^{H} (<\delta \phi_{k}(\textbf{r}'') | \phi_{k}(\textbf{r}'') > + <\phi_{k}(\textbf{r}'') | \phi_{k}(\textbf{r}'') > )
\end{split}
\end{equation}


Therefore, the variation in \textbf{Eq. \ref{haa}} with respect to $\phi^*_{k}(\textbf{r}'')$ is given as 

\begin{equation}\begin{split}\label{hha1}
& \frac{\delta(E^{\textit{H}} - \sumi<i> E_{i}^{H} (<\bwff<i><> | \bwff<i><> > -1))}{\delta \phi^*_{k}(\textbf{r}'')}  \\
& = (-\frac{\nabla^{2}_{k}}{2} + V_\textit{ext}(\textbf{r}'')+ \sum_{i}  <\bwfn<i><>\ | \frac{1}{|\textbf{r} - \textbf{r}^{''} |} \ | \bwfn<i><>>) \phi_{k}(\textbf{r}'') = E_{k}^{H} \phi_{k}(\textbf{r}'') \\
& = (-\frac{\nabla^{2}_{i}}{2} + V_\textit{ext}(\textbf{r})+ \sum_{j}  <\bwfn<j><'>\ | \frac{1}{|\textbf{r} - \textbf{r}^{'} |} \ | \bwfn<j><'>>) \bwff<i><> = E_{i}^{H} \bwff<i><>,
\end{split}
\end{equation}

where $E_{i}^{H}$  also can be treated as the energy. The \textbf{Eq. \ref{hha1}} is a group of dependent single particle equations. One will find out this equation is self-consistent and can be solved using iteratively.
...
\begin{matrix}
    ...
    \bwff<2><1> & \bwff<2><2> & \cdots & \bwff<2><N> \\
    \vdots               & \vdots               &        & \vdots               \\
...
The Hartree and Hartree-Fock (HF) methods are very classic methods to solve the many-electron Schrödinger equation. However, the HF method only includes the exchange term, not the electron correlation term, which is not suitable 
for the solid materials. Apart from the Hartree and HF method, there is a modern method to solve the more complicated calculation of electrons' system, namely the DFT. It is introduced by Hohenberg and Kohn in 1964, moreover, 
Kohn and Pople was awarded Chemistry Nobel Prize in 1998.

The idea of the DFT is to treat the electron density of solid instead of using the many-particle wavefunction. One can benefit that the degree of freedom reduces from 3N (N is the number of electrons) to 3. It is apparently less 
complicate than those of Hartree and HF during calculations. 
...
There are two questions coming out if considering the electron density as the role of wavefunction. The first one is whether it
 is the equivalence relation between the electron density and wavefunction in the system. The second one is how to solve this 
...
Taking advangtage of the Hamitonian from \textbf{Eq. \ref{both}}, one can get
...
Another similar inequality equation will be gained if one changes the equation $\langle \Psi_1\ |\hat{H}_{2}|\ \Psi_1 \rangle$ like \textbf{Eq. \ref{hkpf2}}
...
many-electron problem to single-electron problem by using different wavefunctions. The DFT only consider to take use of information from Hamitonian, however not solvable. 
...
In order to derive the ground state properties in the above system, one can view this problem as the process of minimizing the total energy by varying the wavefunction $\Psi^{\textit{KS}*}_{\textit{i}}(\textbf{r})$. Since
...
\sumi<i> \Psi^{\textit{KS}*}_{\textit{i}}(\textbf{r}) (-\frac{\nabla^{2}_{i}}{2}\ + \ V^\textit{KS}(\textbf{r})) \Psi^{\textit{KS}}_{\textit{i}}(\textbf{r}) = \sumi<i> \Psi^{\textit{KS}*}_{\textit{i}}(\textbf{r}) E^{\textit{KS}}_{\textit{i}} \Psi^{\textit{KS}}_{\textit{i}}(\textbf{r}).
\end{equation}
...
There are more modern methods to approximate the exchange-correlation energy, such as: the optimized effective potential
...
equation and Hartree or Hartree-Fock equation is that the wavefunction is replaced by the electron density. Thereby, an initial 
electron density is defined by some way at the beginning of the calculation.  Afterwareds, the equation is solved iteratively until the reasonable solution is obtained.
...
In order to solve \textbf{Eq. (\ref{aaa})}, The Kohn-Sham equation can be transformed into the general eigenvalue problem. Now if the Kohn-Sham equation is given as (ignore the index compared with \textbf{Eq. \ref{aaa}})
...
A one-to-one map $y_n = g(x_n)$ is defned, where $g(x_n)$ has $N$ finite number of variables. In order to be utilized in physics, the $g(x_n)$ should be allowed all the real numbers in certain interval, such as $[x_0, x_N]$, that is,
